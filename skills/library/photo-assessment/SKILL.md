...
Photos are useful and routinely over-trusted. Your job is to extract the real signal and
be explicit about the noise.

## What photos are good for
- Progress comparison across 6+ weeks under the standard protocol
- Regional fat distribution (informs expectations about where it comes off last)
- Proportion and structure (informs exercise selection and emphasis)
- Posture and obvious asymmetry (informs movement screening, not diagnosis)

## What photos are not good for
- **Body fat percentage.** Visual estimates carry roughly ±5–8 percentage points of
  error. That range spans "keep cutting" and "start eating." State a range, state the
  error, never state a single number.
- Detecting change under six weeks. That's lighting, water, glycogen, and pump.
- Anything medical.

Say this once, clearly, the first time. Don't re-litigate it every upload.

## Progress photo review

1. Check protocol compliance first (`photos/PROTOCOL.md`). Different light or different
   posture means the comparison is void. Say so rather than reading tea leaves.
2. Compare to the oldest available compliant photo, not the most recent one.
3. Cross-reference against the 7-day weight average and the waist measurement **before**
   giving a verdict. When photos and tape disagree, the tape wins.
4. Report specifically: where change is visible, where it isn't, what that implies.
   "Waist is down, upper back looks the same" is useful. "Looking great, keep it up" is
   worthless.
5. If there's no visible change and the numbers say there should be, say that too. And
   if the numbers agree there's been no change, lead with that — don't find something
   nice in the photo to soften it.
...
- **Lean mass required.** Estimate the difference between his current lean mass and what
  ...
  - Advanced (3+ yrs): ~0.2 kg/month or less
  - **In a deficit: near zero for anyone past the novice stage.** This matters enormously
    given the current goal. If the target requires meaningful new muscle, that is a
    separate phase after this one, and the honest answer is "this is a two-phase,
    18-month project, not a 12-week one."
- **Body fat level required.** Most "aesthetic" reference physiques sit around 8–12%
  (male) or 18–22% (female). Name the number, and name the lifestyle cost of holding it
  year-round versus hitting it for a photo.
