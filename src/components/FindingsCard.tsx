 ...
 *
 * Working through their four examples told us exactly what was wrong:
 *
 *   "Calorie target is 134 kcal above the RMR floor"  -> a check that PASSED. Nothing is wrong,
 *       there is no action, and it is on their screen. A non-event is noise.
 ...
 *   "A promotion trigger has an unfilled threshold"   -> a QUESTION FOR THEM, on a page they cannot
 *       answer on. If the system needs something from them the coach asks, in conversation.
 *   "Something in the chart is dated 2026-08-17"      -> an internal integrity message, leaked to
 *       the user, and vague enough to be unactionable even if it were their.
 *
 * So this component renders only `audience: 'athlete'` findings (scripts/lib/findings.mjs), the
 * three-part test being: it is true about THEM, it changes what they do or whether they can trust
 * this page, and the action is available from where they are standing. Everything else goes to the
 ...
 * On their phone the headline is the whole message; if it needs a paragraph it is not an alert.
 *
 ...
 * permanent reassurance panel is a thing they learn to scroll past, which is how the one alert that
